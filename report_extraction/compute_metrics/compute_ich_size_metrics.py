...
def build_output_paths(
    ...
    output_dir = os.path.join(
        output_root, f"prompt{prompt_id}", model_name, "ich_size", mode
    )
    os.makedirs(output_dir, exist_ok=True)

    csv_name = f"metrics_{model_name}_prompt{prompt_id}_ich_size_{mode}{suffix}.csv"
    txt_name = f"errors_{model_name}_prompt{prompt_id}_ich_size_{mode}{suffix}.txt"
...
def compute_metrics_for_file(
    ...
    if run_strict:
        ...
        output_csv, output_txt = build_output_paths(
            formatted_results_path, output_root, "strict", suffix
        )
    ...
    if run_flexible:
        ...
        output_csv, output_txt = build_output_paths(
            formatted_results_path, output_root, "flexible", suffix
        )
        metrics_df.to_csv(output_csv, index=False)
