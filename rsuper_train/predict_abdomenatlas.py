...
# Ensure caches/config are writable on cluster nodes before importing matplotlib.
_cache_root = os.environ.get("RSUPER_CACHE_DIR", f"/tmp/rsuper_cache_{os.getuid()}")
_mpl_dir = os.path.join(_cache_root, "matplotlib")
_xdg_cache = os.path.join(_cache_root, "xdg_cache")
_xdg_config = os.path.join(_cache_root, "xdg_config")
for _d in (_cache_root, _mpl_dir, _xdg_cache, _xdg_config):
...
def init_model(args,classes,old_classes=None):
    ...
    for ckp_path in args.load:
        ...
        if not args.EMA:
            pth = checkpoint['model_state_dict']
        else:
            pth = checkpoint['ema_model_state_dict']
        try:
            pth = pth.state_dict()
        except:
            pass
